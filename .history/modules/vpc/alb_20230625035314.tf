...
resource "helm_release" "alb_controller" {
  ...
  depends_on = [ aws_eks_node_group.node_group,
        ...
        kubernetes_service_account.eks_serviceaccount ]
}
    ...
    Statement = [
      {
        Effect = "Allow"
        Principal = {
          Federated = "${aws_eks_cluster.cluster.identity[0].oidc[0].issuer}"
        }
...
resource "kubernetes_service_account" "eks-serviceaccount" {
  metadata {
